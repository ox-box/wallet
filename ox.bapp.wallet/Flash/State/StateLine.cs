...
{
    public partial class StateLine : DarkDocument, INotecaseTrigger, IModuleComponent
    {
        ...
        LatestTagFlowControl LatestButton;
        HotTagFlowControl HotButton;
        /// <summary>
        /// 0:latest
        /// 1:hot
        /// 2:tag
        /// </summary>
        public int TagKind { get; set; } = 0;
        #region Constructor Region

        public StateLine()
        {
            InitializeComponent();
            this.DockText = UIHelper.LocalString("闪态流", "Flash State Flow");
            ...
            this.VisibleChanged += StateLine_VisibleChanged;
        }

        private void StateLine_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                tagRange = 1;
        ...
        {
            this.LatestButton.TagControl_Click(null, null);
        }
        ...
        public void ReloadFlashStates()
        {
            if (this.CurrentFunc.IsNotNull())
            {
                this.DoInvoke(() =>
                {
                    ...
                    this.fsHash.Clear();
                    this.senderHash.Clear();
                    var fss = this.CurrentFunc(this.PageIndex);
                    if (fss.IsNotNullAndEmpty())
                    {
                        foreach (var fsh in fss)
                        {
                            ...
                            html += $"</p>";
                            html += "</div>";
                            var senderSH = fsh.FlashState.Author;
        ...
        void reloadTags()
        {
            this.TagsPanel.Controls.Clear();
            var ts = FlashMessageProvider.Instance.FlashStateTagCount.OrderByDescending(m => m.Value.Value).Take(10 * tagRange);
            this.LatestButton = new LatestTagFlowControl(this);
            this.TagsPanel.Controls.Add(this.LatestButton);
            this.HotButton = new HotTagFlowControl(this);
            this.TagsPanel.Controls.Add(this.HotButton);
            foreach (var tag in ts)
            {
                this.TagsPanel.Controls.Add(new TagFlowControl(this, tag.Key));
            }
        }
        ...
        private void FSSender_Click1(object sender, HtmlElementEventArgs e)
        {
            ...
            (this.Module as FlashMessageModule).OpenSenderHome(he.Id.Split('-')[0]);
        }
        ...
        {
            if (this.TagsPanel.Height < this.Height / 2)
            {
                tagRange++;
                reloadTags();
            }
        ...
        private void TagsPanel_SizeChanged(object sender, EventArgs e)
        {
            var h = this.TagsPanel.Location.Y + this.TagsPanel.Height;
            var location = this.renderer.Location;
            var sh = h - location.Y;
            this.renderer.Location = new System.Drawing.Point(location.X, location.Y + sh);
            this.renderer.Height = this.renderer.Height - sh;
            //this.Validate();
        ...
        private void StateLine_Load(object sender, EventArgs e)
        {
