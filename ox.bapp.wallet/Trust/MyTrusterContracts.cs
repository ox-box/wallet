...
{
    public partial class MyTrusterContracts : DarkToolWindow, INotecaseTrigger, IModuleComponent
    {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (nodes != null && nodes.Length == 1)
                {
                    ...
                    if (nt == 3)
                    {
                        ...
                        sm.Click += Sm_Click;
                        menu.Items.Add(sm);
                        sm = new ToolStripMenuItem(UIHelper.LocalString("复制受托人地址", "Copy Trustee Address"));
                        sm.Tag = node.Tag;
                        sm.Click += Sm_Click1;
                        menu.Items.Add(sm);
                        sm = new ToolStripMenuItem(UIHelper.LocalString("复制受托人公钥", "Copy Trustee Public Key"));
                        sm.Tag = node.Tag;
                        sm.Click += Sm_Click5;
                        menu.Items.Add(sm);
        ...
        private void Sm_Click5(object sender, EventArgs e)
        {
            ...
            var pubkey = p.Value.Trustee.ToString();
        ...
        private void Sm_Click4(object sender, EventArgs e)
        {
            ToolStripMenuItem ToolStripMenuItem = sender as ToolStripMenuItem;
            KeyValuePair<UInt160, AssetTrustContract> p = (KeyValuePair<UInt160, AssetTrustContract>)ToolStripMenuItem.Tag;
            new TransferTrustAsset(this.Operater, p.Value.Truster, p.Key, p.Value).ShowDialog();
        }
        ...
        private void Sm_Click1(object sender, EventArgs e)
        {
            ToolStripMenuItem ToolStripMenuItem = sender as ToolStripMenuItem;
            KeyValuePair<UInt160, AssetTrustContract> p = (KeyValuePair<UInt160, AssetTrustContract>)ToolStripMenuItem.Tag;
            var addr = Contract.CreateSignatureRedeemScript(p.Value.Trustee).ToScriptHash().ToAddress();
            Clipboard.SetText(addr);
            string msg = addr + UIHelper.LocalString("  已复制", "  copied");
            Bapp.PushCrossBappMessage(new CrossBappMessage() { Content = msg, From = this.Module.Bapp });
            DarkMessageBox.ShowInformation(msg, "");
            ...
            foreach (var tx in block.Transactions)
            {
                if (tx is AssetTrustTransaction att)
                {
                    if (att.TrustContract.Equals(Blockchain.TrustAssetContractScriptHash))
                    {
                        ...
                        if (this.Operater.Wallet.ContainsAndHeld(truster))
                        {
                            DoReload();
                        }
            ...
            if (bizPlugin != default)
            {
                this.DoInvoke(() =>
                {
                    foreach (var p in this.Operater.Wallet.GetHeldAccounts())
                    {
                        ...
                        node.NodeType = 1;
                        node.Tag = p;
                        foreach (var ct in bizPlugin.AssetTrustContacts.Where(m => m.Value.Truster.Equals(p.GetKey().PublicKey)))
                        {
                            var subnode = new DarkTreeNode(UIHelper.LocalString($"信托地址: {ct.Key.ToAddress()}", $"Trust Address: {ct.Key.ToAddress()}"));
                            subnode.NodeType = 3;
                            subnode.Tag = ct;
                            var trusteeAddr = Contract.CreateSignatureRedeemScript(ct.Value.Trustee).ToScriptHash().ToAddress();
                            var n3 = new DarkTreeNode(UIHelper.LocalString($"受托人地址: {trusteeAddr}", $"Trustee Address: {trusteeAddr}"));
                            n3.NodeType = 3;
                            n3.Tag = ct;
